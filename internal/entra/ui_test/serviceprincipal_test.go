...
func TestHtmxSPCreate(t *testing.T) {
	...
	if w.Code != http.StatusOK {
		t.Errorf("GET new: expected status %d, got %d. Body: %s", http.StatusOK, w.Code, w.Body.String())
	}
	body := w.Body.String()
...
func TestHtmxSPEdit(t *testing.T) {
	...
	if w.Code != http.StatusOK {
		t.Errorf("GET edit: expected status %d, got %d. Body: %s", http.StatusOK, w.Code, w.Body.String())
	}
	body := w.Body.String()
