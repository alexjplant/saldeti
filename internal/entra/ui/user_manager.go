...
func UserSetManagerHandler(h *UIHandler) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		if err != nil {
			log.Error().Err(err).Msg("Failed to authenticate for setting manager")
			h.handleManagerResponse(c, id, FlashDanger, "Failed to authenticate. Please try again.")
			return
		}
		...
		if err != nil {
			log.Error().Err(err).Msg("Failed to set manager")
		...
		if resp.StatusCode != http.StatusNoContent {
			...
			log.Error().Int("status", resp.StatusCode).Str("body", string(respBody)).Msg("Failed to set manager")
			h.handleManagerResponse(c, id, FlashDanger, "Failed to set manager. Please try again.")
			return
		}
...
func UserRemoveManagerHandler(h *UIHandler) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		if err != nil {
			log.Error().Err(err).Msg("Failed to authenticate for removing manager")
			h.handleManagerResponse(c, id, FlashDanger, "Failed to authenticate. Please try again.")
			return
		}
		...
		resp, err := httpClient.Do(req)
		if err != nil {
			log.Error().Err(err).Msg("Failed to remove manager")
		...
		if resp.StatusCode != http.StatusNoContent {
			respBody, _ := io.ReadAll(resp.Body)
			log.Error().Int("status", resp.StatusCode).Str("body", string(respBody)).Msg("Failed to remove manager")
			h.handleManagerResponse(c, id, FlashDanger, "Failed to remove manager. Please try again.")
			return
		}
