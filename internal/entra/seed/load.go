...
func validateConfig(cfg *SeedConfig) error {
	...
	// Build set of all SP app IDs for validation (from explicit SPs + auto-created from Applications)
	spAppIDs := make(map[string]bool)
	for _, sp := range cfg.ServicePrincipals {
		spAppIDs[sp.AppID] = true
	}
	for _, app := range cfg.Applications {
		if app.AppID != "" {
			spAppIDs[app.AppID] = true
		}
	}

	// Validate MemberSPAppIDs on groups
	for i, group := range cfg.Groups {
		for _, appID := range group.MemberSPAppIDs {
			if !spAppIDs[appID] {
				return fmt.Errorf("group[%d]: member_sp_app_ids %s does not reference any service_principal", i, appID)
			}
		}
	}

	// Validate OwnerUPNs on service principals
	for i, sp := range cfg.ServicePrincipals {
		for _, upn := range sp.OwnerUPNs {
			if !userEmails[upn] {
				return fmt.Errorf("service_principals[%d]: owner_upns %s does not reference any user", i, upn)
			}
		}
	}
	...
	for i, assignment := range cfg.AppRoleAssignments {
		...
		case "", "user":
			if assignment.PrincipalName == "" {
				...
				if !userEmails[assignment.PrincipalName] {
					return fmt.Errorf("app_role_assignment[%d]: principal_name %s does not reference any user", i, assignment.PrincipalName)
				}
			}
		case "group":
			if assignment.PrincipalName == "" {
				return fmt.Errorf("app_role_assignment[%d]: principal_name is required when principal_type is 'group'", i)
			}
			if !groupNames[assignment.PrincipalName] {
				return fmt.Errorf("app_role_assignment[%d]: principal_name %s does not reference any group", i, assignment.PrincipalName)
			}
		case "service_principal":
			if assignment.PrincipalName == "" {
				return fmt.Errorf("app_role_assignment[%d]: principal_name is required when principal_type is 'service_principal'", i)
			}
			if !spAppIDs[assignment.PrincipalName] {
				return fmt.Errorf("app_role_assignment[%d]: principal_name %s does not reference any service_principal", i, assignment.PrincipalName)
			}
		default:
			return fmt.Errorf("app_role_assignment[%d]: invalid principal_type %q", i, assignment.PrincipalType)
		}
	}
...
func SeedFromConfig(s store.Store, cfg *SeedConfig) error {
	...
	// Process SP owner_upns
	for _, seedSP := range cfg.ServicePrincipals {
		if len(seedSP.OwnerUPNs) == 0 {
			continue
		}
		sp, err := s.GetServicePrincipalByAppID(ctx, seedSP.AppID)
		if err != nil {
			return fmt.Errorf("failed to find SP for owner assignment with appId %s: %w", seedSP.AppID, err)
		}
		for _, upn := range seedSP.OwnerUPNs {
			userID, ok := upnToID[upn]
			if !ok {
				return fmt.Errorf("service_principal owner_upn %s does not reference any user", upn)
			}
			if err := s.AddSPOwner(ctx, sp.ID, userID, "user"); err != nil && !errors.Is(err, store.ErrAlreadySPOwner) {
				return fmt.Errorf("failed to add owner %s to SP %s: %w", upn, seedSP.AppID, err)
			}
		}
	}

	// Process member_sp_app_ids from groups (must be after SPs are created)
	for _, group := range cfg.Groups {
		groupID := groupNameToID[group.DisplayName]
		for _, spAppID := range group.MemberSPAppIDs {
			sp, err := s.GetServicePrincipalByAppID(ctx, spAppID)
			if err != nil {
				return fmt.Errorf("failed to find SP for member_sp_app_id %s in group %s: %w", spAppID, group.DisplayName, err)
			}
			if err := s.AddMember(ctx, groupID, sp.ID, "servicePrincipal"); err != nil && !errors.Is(err, store.ErrAlreadyMember) {
				return fmt.Errorf("failed to add SP %s to group %s: %w", spAppID, group.DisplayName, err)
			}
		}
	}
	...
	for i, assignment := range cfg.AppRoleAssignments {
		...
		case "", "user":
			if assignment.PrincipalName != "" {
				var ok bool
				principalID, ok = upnToID[assignment.PrincipalName]
				if !ok {
					return fmt.Errorf("app_role_assignment[%d]: principal_name %s does not reference any user", i, assignment.PrincipalName)
				}
			} else {
		...
		case "group":
			var ok bool
			principalID, ok = groupNameToID[assignment.PrincipalName]
			if !ok {
				return fmt.Errorf("app_role_assignment[%d]: principal_name %s does not reference any group", i, assignment.PrincipalName)
			}
		case "service_principal":
			sp, err := s.GetServicePrincipalByAppID(ctx, assignment.PrincipalName)
			if err != nil {
				return fmt.Errorf("app_role_assignment[%d]: failed to find SP for %s: %w", i, assignment.PrincipalName, err)
			}
			principalID = sp.ID
		default:
			return fmt.Errorf("app_role_assignment[%d]: invalid principal_type %q", i, assignment.PrincipalType)
