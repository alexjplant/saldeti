...
func (s *memoryStore) CreateApplication(ctx context.Context, app model.Application) (model.Application, error) {
	...
	// Process owners from owners@odata.bind
	for _, ownerRef := range app.Owners {
		...
		if len(parts) > 0 {
			...
			s.appOwners[app.ID][objectID] = objectType
		}
	}
...
func (s *memoryStore) DeleteServicePrincipal(ctx context.Context, id string) error {
	...
	// Clean up stale SP references in group membership maps
	for _, memberMap := range s.members {
		delete(memberMap, id)
	}

	// Clean up stale SP references in group owner maps
