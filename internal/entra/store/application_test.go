...
func TestCreateApplicationWithOwners(t *testing.T) {
	ctx := context.Background()
	s := NewMemoryStore()
	...
	// Create application with owners via Owners field (simulates owners@odata.bind)
	app := model.Application{
		DisplayName: "App With Owners",
		Owners: []model.DirectoryObjectRef{
	...
	// Verify owners are stored
	owners, count, err := s.ListApplicationOwners(ctx, created.ID, model.ListOptions{})
	require.NoError(t, err)
	assert.Equal(t, 1, count)
	assert.Len(t, owners, 1)
	assert.Equal(t, user.ID, owners[0].ID)
	assert.Equal(t, "#microsoft.graph.user", owners[0].ODataType)
	assert.Equal(t, "Owner User", owners[0].DisplayName)
}
