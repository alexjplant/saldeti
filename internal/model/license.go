...
var (
	defaultSkus     []SubscribedSku
	defaultSkusOnce sync.Once
)
...
// FindServicePlanName looks up a service plan name by its skuPartNumber and planID from the default catalog
func FindServicePlanName(skuPartNumber, planID string) (planName string, found bool) {
	for _, sku := range DefaultSubscribedSkus() {
		if sku.SkuPartNumber == skuPartNumber {
			for _, plan := range sku.ServicePlans {
				if plan.ServicePlanID == planID {
					return plan.ServicePlanName, true
				}
			}
		}
	}
