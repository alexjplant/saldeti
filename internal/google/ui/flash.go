...
const flashCookieName = "saldeti_google_flash"
const flashContextKey = "google_flash_read"

func SetFlash(c *gin.Context, level FlashLevel, message string) {
	value := url.QueryEscape(string(level) + ":" + message)
	c.SetSameSite(http.SameSiteLaxMode)
	c.SetCookie(flashCookieName, value, 60, "/google-ui", "", false, true)
}
...
func GetFlash(c *gin.Context) *Flash {
	...
	c.SetSameSite(http.SameSiteLaxMode)
	c.SetCookie(flashCookieName, "", -1, "/google-ui", "", false, true)
